...
class BenchmarkTest
{
	...
	static function getTree()
	{
		...
		$frameSizeSmall = $frame->getClone()->setPosn(0, 0, 1)->setSizen(13, 13)->appendChild(Quad::create()->setSizen(13,
				13)->setBgcolor('fff5'));
		...
		$cases[1]->appendChild($frame->getClone()->appendChild($quad->getClone()->setAlign('center',
					'center')));
		...
		$cases[3]->appendChild($frameSize->getClone()->appendChild($quad->getClone()->setAlign('center',
					'center')));

		$cases[4]->appendChild($title->getClone()->setText('5. Frame + Size + Quad + Relative Align'));
		$cases[4]->appendChild($frameSize->getClone()->setPosn(10, -10)->appendChild($quad->getClone()->setRelativeAlign('right',
					'bottom')));

		$cases[5]->appendChild($title->getClone()->setText('6. Frame + Size + Quad + Relative Align + Align'));
		$cases[5]->appendChild($frameSize->getClone()->appendChild($quad->getClone()->setRelativeAlign('right',
					'bottom')->setAlign('right', 'bottom')));
		...
		$cases[7]->appendChild($frameSizeAlign->getClone()->appendChild($quad->getClone()->setRelativeAlign('right',
					'bottom')->setAlign('right', 'bottom')));
...
class BenchmarkCommand extends Command
{
	...
	protected function execute(InputInterface $input, OutputInterface $output)
	{
		...
		for($i = 0; $i < $runs; $i++)
		{
			...
			$output->writeln(sprintf('Run %d, time= %sms, memory=%s', $i, $event->getDuration(),
					$event->getMemory()));
